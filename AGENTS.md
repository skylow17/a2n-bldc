...
|---|---|
| `PB13 = SPI2_MOSI`, `PB15 = SPI2_SCK` | Impossible : en AF5 sur ce boîtier, `PB13` ne peut être que SCK et `PB15` que MOSI. **La carte a été retouchée** (pistes coupées, liaisons refaites par fil) et le SPI matériel fonctionne avec le brochage du v1. Le schéma reste à corriger avant toute nouvelle fabrication, sinon le défaut revient. **Point fragile de cette retouche :** le fil `SCLK` doit être soudé **côté `U3` de la coupure** de piste. Le 2026-09-26 il l'était de l'autre côté, sur le tronçon mort : l'horloge n'arrivait pas au DRV. Si le SPI retombe, penser d'abord à ce fil, soudé du mauvais côté ou dessoudé. **Signature :** tous les registres se relisent à `0x000` et `DRV.PROBE` échoue, mais `DRV.NCS` rend `ALIVE`, et écrire `DRV.REG 6 083` ne fait pas monter les trois `SOx` (`ADC?`, `csa_raw`) vers ~3740 counts. Méthode complète dans `STATUS.md`, « Le SPI du DRV ne répond plus ». |
