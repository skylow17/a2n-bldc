...
| `VREF.BUF ON` / `OFF` | `OK csr=<hex> ready=<0\|1>` | Branche le tampon de référence interne du MCU sur `VREF+`, réglé sur 2,048 V — la tension même que vise le MCP1501, pour qu'aucune source ne tire contre l'autre. Diagnostic : si le balayage s'arrête net, la broche n'était tenue par personne. **Ne pas laisser actif** tant que la liaison vers `U5` n'est pas tranchée |
