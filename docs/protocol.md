...
| `VREF.SCAN` | `OK held=<0\|1> vrefint_min=<c> vrefint_max=<c> vrefint_mean=<c> vref_min_mv=<mV> vref_max_mv=<mV> vref_mean_mv=<mV> seq=<12 bruts> v3v3_min=<c> v3v3_max=<c> v3v3_mean=<c>` | 64 conversions serrées de VREFINT, puis 64 du diviseur 3V3. Une référence saine donne quelques LSB d'écart ; deux voies dispersées dans la même proportion disent que c'est VREF+ qui bouge, et non la lecture. `seq` distingue un nuage aléatoire d'une sinusoïde repliée |
