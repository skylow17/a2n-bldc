...
| `VREF.BUF ON [<mv>]` / `OFF` | `OK csr=<hex> ready=<0\|1> nominal_mv=<mv>` | Branche le tampon de référence interne du MCU sur `VREF+`. `<mv>` vaut `2048` (défaut), `2500` ou `2900` — les trois échelles du VREFBUF. À 2048 il vise la tension même que le MCP1501, pour qu'aucune source ne tire contre l'autre ; `2900` est la seule échelle au-dessus du seuil de sous-tension de 2,6 V de la broche `VREF` du DRV8304, et sert à tenter de réveiller les amplis de shunt sans fer à souder. Sur cette carte l'essai est non concluant : `ready` reste à 0, le MCP1501 (±5 mA) tient le nœud. Diagnostic : si le balayage s'arrête net, la broche n'était tenue par personne. **Ne pas laisser actif** tant que la liaison vers `U5` n'est pas tranchée |
