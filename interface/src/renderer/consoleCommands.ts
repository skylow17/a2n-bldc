...
export const CONSOLE_GROUPS: ConsoleGroup[] = [
  ...
  },
  {
    ...
    commands: [
      ...
      { verb: 'PWM', syntax: 'PWM ON | OFF | <a> <b> <c>', summary: 'Enables or cuts the outputs, or sets the three duty cycles in per mille. Enabling refuses on a latched fault or an absent host.', arms: true },
    ],
  },
  {
  ...
  },
  {
    ...
    commands: [
      ...
      { verb: 'IMOT?', syntax: 'IMOT?', summary: 'Working offsets of the current chain, plus the last raw and centred reading. Says whether the offsets were measured or only assumed.' },
      { verb: 'IMOT.CAL', syntax: 'IMOT.CAL [<n>]', summary: 'Offset campaign with the DRV CAL pin raised, which shorts the amplifier inputs and gives the chain zero. Stores the result and reports the noise.' },
      { verb: 'IMOT.NOISE', syntax: 'IMOT.NOISE [<n>]', summary: 'The same measurement without touching CAL and without storing: the chain as it works. The gap between the two is the useful part.' },
